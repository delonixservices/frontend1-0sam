...
export class HotelAutosuggestComponent implements OnInit, OnDestroy {
  ...
  onSuggestionAdded(suggest) {
    if (!suggest) {
      return; // Early return if no suggestion
    }
  ...
  loadDestination() {
    ...
    const data = concat(
      of([]),
      this.suggestionsInput.pipe(
        debounceTime(800), // Debounce to limit API calls
        distinctUntilChanged(),
        tap(() => {
          this.suggestionsLoading = true; // Show loading indicator
          this.page = 0;
          this.currentItemsCount = 0;
          this.items = [];
        }),
        switchMap(term => {
          ...
          return this.getData().pipe(
            catchError(() => {
              this.suggestionsLoading = false; // Hide loading indicator
    ...
    data.subscribe((res) => {
      this.suggestions = res.map(item => ({
        name: item.name,
        city: item.city || '', // Ensure city is available
        type: item.type,
