...
//   }
//   addFilterToAppliedList(filter: any) {
//     this.appliedFilters.push(filter);
//   }

//   removeFilter(filter: any) {
//     // Remove the selected filter from the applied filters list
//     const index = this.appliedFilters.indexOf(filter);
//     if (index !== -1) {
//       this.appliedFilters.splice(index, 1);
//       // Add logic to update the filter state and reapply filters here
//       // this.sideBarFilter();
//     }
//   }
...
//     this.appliedFilters = [];
//     // Add logic to clear filter state and reapply filters here
//     // this.sideBarFilter();
//   }

//   ngOnInit() {
//     this.sharedService.setResponseData(this.allHotel);
...
//     this.searchHotels(queryParams).pipe(takeUntil(this.ngUnsubscribe))
//       .subscribe((response) => {
...
export class HotelSearchComponent implements OnInit, OnDestroy {
  ...
  totalHotelsCount: number;
  totalPropertiesCount: number = 1374; // Example total property count, update dynamically
  currentHotelsCount: number;
  page: number = 1;
  perPage: number = 15;
  totalPages: number;
  ...
  ngOnInit() {
    ...
    this.getTotalPropertiesCount(); // Initialize total property count
  }
  ...
  parseQueryParams(params) {
    ...
    this.hotelsearchkeys = {
      ...
      details: typeof params.details === 'string' ? JSON.parse(params.details) : [{
        room: '1',
        adult_count: '1',
        child_count: '0',
        children: []
  ...
  // Get total properties count (replace with API logic)
  getTotalPropertiesCount() {
    // Example logic for dynamic total count update
    this.totalPropertiesCount = 2000; // Update with dynamic data from API if necessary
  }
  ...
  searchResult(hotelsearchkeys, filters = {}) {
    ...
    const queryParams = Object.assign({
      perPage: this.perPage
    }, hotelsearchkeys, { filters });

    this.searchHotels(queryParams).pipe(takeUntil(this.ngUnsubscribe)).subscribe((response) => {
  ...
  handleNoResults() {
    this.filteredHotels = [];
    this.copyFilteredHotels = [];
    this.norecordfoundtitle = 'There is no result available currently. Please search for other hotels';
  }
  ...
  hoteldetails(hotel) {
    ...
    let hoteldetailkeys = { ...this.hotelsearchkeys, area: { ...this.hotelsearchkeys.area, name: hotel.name, id: hotel.id, type: 'hotel' } };
    localStorage.setItem('hoteldetailkeys', JSON.stringify(hoteldetailkeys));
    localStorage.setItem('hotelObj', JSON.stringify(hotel));
    ...
    localStorage.setItem('hoteldetailparams', JSON.stringify(queryParams));
    this.router.navigate(['/hotels', 'hoteldetails'], { queryParams });
  }

  onScroll() {
    if (!this.allowNextIteration) return;
    ...
    this.searchHotels(requestParams).pipe(takeUntil(this.ngUnsubscribe)).subscribe((response) => {
      if (response && response.data) {
        this.filteredHotels = this.filteredHotels.concat(response.data.hotels);
        this.totalHotelsCount = response.data.totalHotelsCount;
        this.currentHotelsCount = response.data.currentHotelsCount;
